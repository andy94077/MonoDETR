...
from lib.models.monodetr.depth_embedding import build_depth_embedding
...
class MonoDepthAttention(nn.Module):
    """ This is the MonoDETR module that performs monocualr 3D object detection """

    def __init__(self, backbone, depthaware_transformer, depth_predictor, bbox_coder, depth_embed, num_classes, num_queries, num_feature_levels,
        ...
        self.depth_embed = depth_embed  # depth and deviation
    ...
    def forward(self, images, calibs, img_sizes, targets) -> Dict[str, Union[torch.Tensor, List[Dict[str, torch.Tensor]], Dict[str, torch.Tensor]]]:
        ...
        for lvl in range(hs.shape[0]):
            ...
            # depth_reg
            depth_pos_embed: torch.Tensor = self.depth_predictor.depth_pos_embed.weight.unsqueeze(0).expand(hs[lvl].shape[0], -1, -1)
            depth_reg = self.depth_embed[lvl](hs[lvl], depth_pos_embed, depth_pos_embed)
            ...
            # depth_ave = (depth_reg[:, :, 0:1] + depth_geo.unsqueeze(-1) + depth_map) / 3
...
def build(model_cfg, loss_cfg):
    ...
    depth_embedding = build_depth_embedding(model_cfg)
    bbox_coder = build_bbox_coder(model_cfg)
    model = MonoDepthAttention(
        backbone,
        depthaware_transformer,
        depth_predictor,
        bbox_coder,
        depth_embed=depth_embedding,
